...
jsmind_path = Path(__file__).parent / "jsmind"
...
class TagMindmap:
    ...
    def save_as_jsmind(self, output_file_path, include_notes):
        jsmind = self._to_jsmind(include_notes=include_notes)
        ...
        template = re.sub(
            "<!-- IMPORTS -->[\w\W]*<!-- IMPORTS -->",
            lambda _: inlined,  # pass string wrapped in lambda to prevent sub processing escape characters
            template,
        )
    ...
    def _to_jsmind(self, topic=None, tree=None, depth=-1, include_notes=False, path=""):
        ...
        if topic is None:
            ...
            data = self._to_jsmind(topic, subtree, 0, include_notes, topic)
            return {
                "meta": {
                    "name": "jsMind remote",
    ...
    def _paths(self):
        return reversed(
            sorted(
                [path for path in self.all_paths if self._starts_with_root_path(path)],
    ...
    def _starts_with_root_path(self, path):
        return path == self.root_path or path.startswith(
            self.root_path + self.seperator
        )
    ...
    def _with_root_path(self, path):
        ...
        return self.root_path + (
            self.seperator + path_from_root if path_from_root else ""
        )
